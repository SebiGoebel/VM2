...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (loadingHumans)
    {
        ...
        m_doctorOperation = shapes::createMeshFromResource("package://moving_ur3e_ros2/collisionObjects/daes/human.dae", scale_doctorOperation);
        ...
        m_patient = shapes::createMeshFromResource("package://moving_ur3e_ros2/collisionObjects/daes/human.dae", scale_patient);
    ...
    // --- adding constraint space from stl to visualize it ---

    if (visualizeArea)
    {
        std::vector<moveit_msgs::msg::CollisionObject> collision_objects7;

        moveit_msgs::msg::CollisionObject co_constraint;
        co_constraint.header.frame_id = move_group_interface.getPlanningFrame();
        co_constraint.id = "constraint_mesh";
        move_group_interface.getPlanningFrame();

        Eigen::Vector3d scale_constraint(0.01, 0.01, 0.01); // 0.001
        shapes::Mesh *m_constraint = shapes::createMeshFromResource("package://moving_ur3e_ros2/collisionObjects/stls/tube.STL", scale_constraint);

        shape_msgs::msg::Mesh mesh_constraint;
        shapes::ShapeMsg mesh_msg_constraint;
        shapes::constructMsgFromShape(m_constraint, mesh_msg_constraint);
        mesh_constraint = boost::get<shape_msgs::msg::Mesh>(mesh_msg_constraint);

        geometry_msgs::msg::Pose constraint_pose;
        constraint_pose.position.x = -1.5; // -0.25
        constraint_pose.position.y = -3;   // -0.35
        constraint_pose.position.z = 0;    // 0.25
        constraint_pose.orientation = euler2Quaternion_deg(-77, 0, -12);

        co_constraint.meshes.push_back(mesh_constraint);
        co_constraint.mesh_poses.push_back(constraint_pose);
        co_constraint.operation = co_constraint.ADD;

        collision_objects7.push_back(co_constraint);

        planning_scene_interface.addCollisionObjects(collision_objects7);

        prompt("Press 'Next' in the RvizVisualToolsGui window to add a constraint from stl");
        draw_title("Adding constraint from stl");
        moveit_visual_tools.trigger();
    }
    // ---------------------------- Adding a stl as Constraint ----------------------------

    // Let's try the simple box constraints first!
    moveit_msgs::msg::PositionConstraint stl_constraint;
    stl_constraint.header.frame_id = move_group_interface.getPoseReferenceFrame();
    stl_constraint.link_name = move_group_interface.getEndEffectorLink();

    Eigen::Vector3d scale_stl(0.01, 0.01, 0.01);
    shapes::Mesh *m_collision = shapes::createMeshFromResource("package://moving_ur3e_ros2/collisionObjects/daes/tube.dae", scale_stl);

    // shape_msgs::msg::Mesh mesh_msgs_constraint;
    shape_msgs::msg::Mesh mesh_collision;
    shapes::ShapeMsg mesh_msg_collision;
    shapes::constructMsgFromShape(m_collision, mesh_msg_collision);
    mesh_collision = boost::get<shape_msgs::msg::Mesh>(mesh_msg_collision);

    stl_constraint.constraint_region.meshes.emplace_back(mesh_collision);

    geometry_msgs::msg::Pose constraint_stl_pose;
    constraint_stl_pose.position.x = -1.5; //-0.25;
    constraint_stl_pose.position.y = -3;   // -0.35;
    constraint_stl_pose.position.z = 0;    // 0.25;
    constraint_stl_pose.orientation = euler2Quaternion_deg(-77, 0, -12);
    stl_constraint.constraint_region.mesh_poses.emplace_back(constraint_stl_pose);
    stl_constraint.weight = 1.0;

    moveit_msgs::msg::Constraints stl_constraints;
    stl_constraints.position_constraints.emplace_back(stl_constraint);
    move_group_interface.setPathConstraints(stl_constraints);
